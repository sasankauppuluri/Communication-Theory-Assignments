...
EbdivN0=2.704; % Obtained from script5
variance=1/(2*EbdivN0); 

bitarr=randbit(12000);
samples=qpskmap(bitarr);
...
for ii=1:len 
         if(real(received_filter(ii))>=0 && imag(received_filter(ii))>=0) 
             received_filter(ii)=1+1i;
         elseif(real(received_filter(ii))>=0 && imag(received_filter(ii))<0)
             received_filter(ii)=1-1i;
         elseif (real(received_filter(ii))<0 && imag(received_filter(ii))>=0)
             received_filter(ii)=-1+1i;
         elseif (real(received_filter(ii))<0 && imag(received_filter(ii))<0)
